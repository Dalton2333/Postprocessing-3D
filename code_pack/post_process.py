...
18/08/2019
Author@Dedao
Postprocessing for 3D topology optimized design.
'''
import os
import sys
import re
...
def rep(string, substitutions):
    '''
    substitutions should be a dict
    '''
...
if __name__ == '__main__':
    ...
    if ap['model_space'] == '2D':
        print("2D postprocessing not included.")

    elif ap['model_space'] == '3D':
        surf_nds, surf_nd_coords = geometry.surface_extractor.get_surface_nodes(ap,part)
        subst = {'the_inp_name':ap['inp_name'],
                 'the_inp_path':ap['inp_path'],
                 'the_part_name':ap['part_name'].upper(),
                 'the_test_dir_path':ap['test_dir_path']}
        copyfile(ap['code_dir_path']+'utilities/abaqus/obj_exporter.py',ap['test_dir_path']+'obj_exporter.py')
        with fileinput.FileInput(ap['test_dir_path']+'obj_exporter.py', inplace=True,backup='.bak') as f:
            for line in f:
                print(rep(line,subst),end='')
            f.close()
        os.chdir(ap['test_dir_path'])
        os.system('abaqus cae noGUI='+ap['test_dir_path']+'obj_exporter.py')
